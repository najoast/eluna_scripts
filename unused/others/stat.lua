...
local itemEntry=60009 --物品id,需要那些可以点击使用的物品，比如炉石之类的
local qtos=0.1   --每完成多少个任务获得一点分配点数
local cz_item=60000   --重置加点所需要的物品材料
local cz_item_count=100  --重置所需要材料数量
...
local function stat_seleGoss(event,player,item,target,intid)
	...
	if (math.modf(questcount:GetUInt32(2)/qtos-points:GetUInt32(1))<=0) then
		player:SendBroadcastMessage("当前剩余属性点数为0，请继续努力~")
		player:GossipComplete()
	else
		if intid==1 then
